...
+(instancetype)cellWithTableView:(UITableView *)tableView messageModel:(CSMessageModel *)model indexPath:(NSIndexPath *)indexPath {
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    self.imageImageView.userInteractionEnabled = YES;
    [self.imageImageView addGestureRecognizer:tap];
//    self.imageImageView.tag = indexPath.row;
    return self;
}
- (void)showBigImageView:(UITapGestureRecognizer *)tap{
    UIImageView *imageView = (UIImageView * )tap.view;
    if (self.delegate && [self.delegate respondsToSelector:@selector(showBigImageIndex:)])
    {
        [self.delegate showBigImageIndex:imageView.tag];
    }
}
#pragma mark - 创建子视图
...
- (void)creatSubViewMessage
{
    ...
    [_messageLabel addGestureRecognizer:singleTap2];
}
...
- (void)setMessageModel:(CSMessageModel *)messageModel {
    ...
    _logoImageView.layer.cornerRadius =_logoImageView.jk_width/2.0;
    
   _bubbleImageView.hidden = YES;
    
    if (messageModel.messageSenderType == MessageSenderTypeMe)
    {
        [_logoImageView sd_setImageWithURL:messageModel.logoUrl placeholderImage:App_IMAGE(@"user_none")];
//        _bubbleImageView.image = [[UIImage imageNamed:@"me"] stretchableImageWithLeftCapWidth:0 topCapHeight:0];
...
//        _bubbleImageView.image = [[UIImage imageNamed:@"me"] stretchableImageWithLeftCapWidth:0 topCapHeight:0];
    ...
    switch (messageModel.messageType)
    {
        case MessageTypeText:
            ...
            _messageLabel.textAlignment =  messageModel.messageSenderType == MessageSenderTypeMe ?NSTextAlignmentLeft: NSTextAlignmentLeft;
        ...
        case MessageTypeVoice:
            ...
            _messageLabel.textAlignment = messageModel.messageSenderType == MessageSenderTypeMe ?NSTextAlignmentCenter: NSTextAlignmentCenter;
        ...
        case MessageTypeImage:
            ...
            _imageImageView.layer.cornerRadius = 8;
            ...
            if (messageModel.imageSmall == nil) {
                imageSize = _imageImageView.frame.size;
                [_imageImageView sd_setImageWithURL:messageModel.imageUrl placeholderImage:nil];
            }
