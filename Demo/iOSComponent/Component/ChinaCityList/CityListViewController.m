...
- (void)viewDidLoad
{
    ...
    UIView *searchView = [[UIView alloc] initWithFrame:CGRectMake(0, App_NavHeight, self.view.frame.size.width, 40*(self.view.frame.size.height/568))];
    ...
    _tableView.frame           = CGRectMake(0,searchView.frame.origin.y+searchView.frame.size.height, self.view.frame.size.width, self.view.frame.size.height-App_NavHeight-App_TabHeight);
...
{
    //自定义导航栏
    UIView *customNavView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, App_NavHeight)];
    customNavView.backgroundColor = BGCOLOR;
    
    UIButton *backBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [backBtn setImage:[UIImage imageNamed:@"cancel"] forState:UIControlStateNormal];
    backBtn.frame = CGRectMake(20, App_StatusBarHeight+7, 30, 30);
    [backBtn addTarget:self action:@selector(back:) forControlEvents:UIControlEventTouchUpInside];
    [customNavView addSubview:backBtn];
    UILabel *titleLab = [[UILabel alloc]initWithFrame:CGRectMake(0, App_StatusBarHeight, customNavView.frame.size.width, customNavView.frame.size.height-App_StatusBarHeight)];
